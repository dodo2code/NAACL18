...
#w1_penu = tf.Variable(tf.truncated_normal([conf.deco_hdim, conf.deco_hdim], 0, 0.1), dtype=tf.float32, name='w1_penu')
#w2_penu = tf.Variable(tf.truncated_normal([conf.deco_hdim, conf.deco_hdim], 0, 0.1), dtype=tf.float32, name='w2_penu')
#w3_penu = tf.Variable(tf.truncated_normal([conf.enco_hdim, conf.deco_hdim], 0, 0.1), dtype=tf.float32, name='w2_penu')
#w_softmax = tf.Variable(tf.truncated_normal([conf.deco_hdim, conf.vocab_size], 0, 0.1), dtype=tf.float32, name='w_softmax')

#w_doc _repeat [300, 200]
w_doc = tf.Variable(tf.truncated_normal([conf.deco_hdim+2*conf.enco_hdim+conf.query_hdim, conf.deco_hdim], 0, 0.1), dtype=tf.float32,name='w_que')
#v_doc _repeat [200, 1]
v_doc = tf.Variable(tf.truncated_normal([conf.deco_hdim, 1], 0, 0.1), dtype=tf.float32,name='v_que')
...
with tf.variable_scope('time_step'):
    for time_step in range(conf.sum_max_l):
        ...
        if time_step == 0:
            ...
            att_context, all_word_att = attention(encoder_final_state, 
                                      w_doc, 
                                      v_doc, 
                                      encoder_hiddens, 
                                      doc_mask_tf, 
            ...
            all_word_attentions.append(all_word_att)
            ...
            att_context, all_word_att = attention(states[-1], 
                                      w_doc, 
                                      v_doc, 
                                      encoder_hiddens, 
                                      doc_mask_tf, 
            ...
            output, state = cell(tf.cond(is_training, lambda: tf.concat([decoder_inputs_embedded[:,time_step-1,:], att_context], 1), \
                                                      lambda: tf.concat([next_input, att_context], 1)), states[-1])
    ...
    all_word_attentions = tf.transpose(tf.convert_to_tensor(all_word_attentions), [1, 0, 2])
...
def save_attention(session, string, rouge_s):
    for m in range(0, nb_batch*conf.batch_size, conf.batch_size):
        s_attention = session.run(all_word_attentions,
        ...
        np.save("./model_base_att/"+string+"sent_base_att_%d_%d"%(m, m+conf.batch_size), s_attention)
...
def pint(pred, gold, is_print=False, is_write=False, name=None, rouge_s=None):
    ...
    if is_write:
        f_res = open('./model_base_att/result'+name+'_R1_%f_R2_%f_RL_%f_R1std_%f_R2std_%f_RLstd_%f'%(rouge_s[0], rouge_s[1], rouge_s[2],rouge_s[3], rouge_s[4], rouge_s[5])+'.txt','w')
            ...
            if k/batch_size%2==0:
                ...
                if rouge_tuple[0]>r1_max:
                    ...
                    if r1_max>0.15:
                        ...
                        model_saver.save(sess,"model_att_best/model_r1_%f_r2_%f_rl_%f"%(rouge_tuple[0], rouge_tuple[1], rouge_tuple[2]))
                ...
                if rouge_tuple[1]>r2_max:
                    ...
                    if r2_max>0.04:
                        model_saver = tf.train.Saver()
                        model_saver.save(sess,"model_att_best/model_r2_%f_r1_%f_rl_%f"%(rouge_tuple[1], rouge_tuple[0], rouge_tuple[2]))
